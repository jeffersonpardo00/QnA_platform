...
const commentsTableInfo = db.prepare(
  "SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'comments'"
).get();

if (commentsTableInfo && !commentsTableInfo.sql.includes('REFERENCES comments(id)')) {
  db.exec(`
    ...
    INSERT INTO comments (id, text, author, like_count, question_id, creation_date, reply_to)
    SELECT
      id,
      text,
      author,
      ...
      creation_date,
      CASE
        WHEN reply_to IS NOT NULL AND reply_to IN (SELECT id FROM comments_old) THEN reply_to
        ELSE NULL
      END AS reply_to
    FROM comments_old;
